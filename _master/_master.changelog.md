...
- [`Feature`] Added some options to optimize informations in Copyrights Infos Array: now is possible to insert Main Author and its E-mail address in two seprate fields, is possible to add fileVersion and appName (see Settings of {app_name}), output of `Authors Names and E-mails` item is now an array;
